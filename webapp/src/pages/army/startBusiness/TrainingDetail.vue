    ...
    <Common-PageContainer>
      <template #content>
        <div v-if="!pagestatus.nodata">
          <div class="container">
            <h2>{{detail.trainingName}}</h2>
            <div class="fair">
              <div class="item">{{detail.publishUnitName}} | {{detail.createTime}}</div>
            </div>
            <ul class="info">
              <li>
                <h3>培训时间</h3>
                <div class="details">
                  <div class="time">{{detail.trainingStartTime}}～{{detail.trainingEndTime}}</div>
                </div>
              </li>
              <li>
                <h3>联系方式</h3>
                <div class="details">
                  <div class="phone">
                    <Common-PhoneCall v-if="detail.contactPhone" :telephone="detail.contactPhone" :underline="true" />
                    <span class="name" v-if="detail.contactPerson">（{{detail.contactPerson}}）</span>
                  </div>
                </div>
              </li>
              <li v-if="detail.trainingType === '2'">
                <h3>机构地址</h3>
                <div class="details">
                  <div class="location" v-if="detail.address">
                    <span class="text" v-if="detail.address">{{detail.address}}</span>
                    <van-button type="info" size="mini" v-if="detail.longitude && detail.latitude" @click="$Common.openMapNav(detail.address, detail.longitude, detail.latitude)">导航</van-button>
                  </div>
                </div>
              </li>
              <li>
...
export default {
  ...
  methods: {
    ...
    async F_loadServiceInfo () {
      ...
      const data = await this.$Api.army.startup.getStartupTrainingDetail(
