<template>
  <div>
        ...
        <van-sticky>
          <van-dropdown-menu
            ...
            <van-dropdown-item
              ...
              :options="data.couponid"
              @change="(value) => { F_changeCoupon(value) }"
            />
          </van-dropdown-menu>
        </van-sticky>
      </div>

      <div class="searchbtn" @click="$router.push('/army/ptreatment/searchmerchant')">
    ...
    <!-- 无数据 -->
    <van-empty v-if="pagestatus.nodata" description="暂无商家信息" />
...
import ListItemMerchant from './ListItemMerchant.vue'
...
export default {
  ...
  watch: {
    ...
    'data.couponId' (id) {
      for (let i = 0; i < this.data.couponid.length; i++) {
        const item = this.data.couponid[i]
        if (item.value === id) {
          this.searchdata.couponId = item.couponId
          break
        }
      }
  ...
  methods: {
    ...
    async F_loadDicCategory () {
      ...
      // 加载优惠券列表
      const couponList = await this.$Api.army.getCouponList({
        start: 0,
        limit: 20,
        status: 0,
      ...
      if (couponList.code === 2000) {
        this.data.couponid.push({
          text: '全部',
          value: '',
          defaultname: '优惠券'
        })

        if (couponList.result && couponList.result.records) {
          couponList.result.records.map((item) => {
            this.data.couponid.push({
              text: item.name,
              value: item.id,
