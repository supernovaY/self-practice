      ...
      <template #content>
        <div v-if="!pagestatus.nodata">
          <div class="container">
            <h2>{{detail.name}}</h2>
            <div class="fair">
              <div class="item">组织单位：{{detail.unit}}</div>
              <div class="item">发布时间：{{detail.createTime}}</div>
            </div>
            <ul class="info">
              <li>
                <h3>招聘会时间</h3>
              ...
              <li>
                <h3>组织形式</h3>
                <div class="details">{{detail.jobFairTypeName}}</div>
              </li>
              <li>
                <h3>联系方式</h3>
                <div class="details">
                  <template v-if="detail.jobFairContactList">
                    <div class="phone" v-for="(item, index) in detail.jobFairContactList" :key="index">
                      <Common-PhoneCall :telephone="item.contactPhone" />
              ...
              <li>
                ...
                <div class="details">
                  <div class="location" v-if="detail.address">
                    ...
                    <van-button v-if="detail.longitude" type="info" size="mini" @click="$Common.openMapNav(detail.address, detail.longitude, detail.latitude)">导航</van-button>
                  </div>
                </div>
              </li>
              <li>
                <h3>招聘会介绍</h3>
                <div class="details" v-html="detail.introduction"></div>
              </li>
            </ul>
          </div>
          <Common-PageFooter />
...
export default {
  ...
  methods: {
    ...
    async getDetail () {
      ...
      const data = await this.$Api.army.job.getJobFairDetail(
...
ul.info{
  ...
  &>li{
    ...
    .location {
      ...
      color: $--color-menugray;
      padding:5px 0 5px 18px;
      background: url("../../../assets/img/businessstart/ic_dw@3x.png") left center/16px no-repeat;
      line-height: 1.25;
      .van-button{
        padding: 2px 10px;
        position: absolute;
