    ...
    <Common-PageContainer @scrollLoad="F_scrollLoad" :showToTop="true">
      ...
      <template #fixbottom>
        <div class="j-m">
          ...
          <van-button type="info" size="large" @click="$router.push('/merchant/service/subaccount/add')">
            新建账号
          </van-button>
...
export default {
  ...
  methods: {
    ...
    async F_delItem (item, index) {
      ...
      const data = await this.$Api.service.delSubAccount({
        id: item.relId,
        serviceId: this.merchantInfo.id
      })
    ...
    async F_loadData () {
      ...
      const data = await this.$Api.service.getSubAccount(
        this.searchdata
      )
