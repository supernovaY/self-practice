  ...
  methods: {
    ...
    async F_loadServiceInfo () {
      ...
      if (data.code === 2000) {
        ...
        // 地区码处理
        const areaCode = this.formdata.areaCode.split(',')
        this.formdata.areaCode = areaCode[areaCode.length - 1]

        // 服务优待处理
        this.formdata.serviceDetails = []
        this.data.merchantinfo.serviceDetails.map((item) => {
          this.formdata.serviceDetails.push(item.key)
        })
    ...
    F_selectPostionCallback (positionData) {
      ...
      this.formdata.latitude = positionData.lat
      this.formdata.longitude = positionData.lng
    },
      ...
      this.$refs.sform.validate()
        .then(() => {
          ...
          this.F_uploadImgs(async () => {
            const submitResult = await this.$Api.service.updateServiceInfo(
              this.formdata
            )
            ...
            if (submitResult.code === 2000) {
              this.$router.back()
              // this.$router.push('/merchant/service/userinfo')
              return
            }
