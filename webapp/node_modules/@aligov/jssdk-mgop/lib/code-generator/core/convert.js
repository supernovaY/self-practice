...
function interfaceToJSONSchema(itf, scope) {
  ...
  function findChildProperties(parentId) {
    return _lodash.default.chain(properties).filter(function (p) {
...
function convert(itf) {
  ...
  var options = Object.assign(Object.assign({}, _jsonSchemaToTypescript.DEFAULT_OPTIONS), {
    bannerComment: ""
  });
  return Promise.all([(0, _jsonSchemaToTypescript.compile)(reqJSONSchema, "Req", options), (0, _jsonSchemaToTypescript.compile)(resJSONSchema, "Res", options)]);
}